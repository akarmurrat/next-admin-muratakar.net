...
export default function Login(){
  ...
  async function onSubmit(e:React.FormEvent){ e.preventDefault(); setLoading(true); setError(null); const {error}=await supabase.auth.signInWithPassword({email,password}); setLoading(false); if(error) setError(error.message); else router.replace('/admin') }
  return (
    <main className="container py-10">
      <div className="card max-w-md mx-auto">
        ...
        <form onSubmit={onSubmit} className="grid gap-3">
          ...
          <button disabled={loading} className="px-4 py-2 rounded-2xl bg-[var(--accent)] text-slate-900 font-bold">{loading?'Giriş yapılıyor…':'Giriş yap'}</button>
        </form>
      </div>
    </main>
  )
